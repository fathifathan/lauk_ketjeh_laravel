    ...
    public function processAuth(Request $request)
    {
        ...
        if ($request->has('login')) {
            ...
            if ($admin && $admin->password === $password) {
                Log::info('Login sebagai admin: ' . $admin->username);
                session([
                    'username' => $admin->username,
                    'password' => $admin->password,
                    'role' => $admin->role
                ]);
                return Redirect::to('/admin/dashboard')->with('flash', 'Login berhasil sebagai admin!');
            }

            $user = DB::table('users')->where('username', $username)->first();
            if ($user && $user->psw_user === $password) {
                session(['username' => $user->username, 'user_id' => $user->user_id, 'role' => 'user']);
